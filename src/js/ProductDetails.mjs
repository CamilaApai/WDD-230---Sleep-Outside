...
export default class ProductDetails {
    ...
    renderProductDetails(selector) {
      document.querySelector(selector).insertAdjacentHTML("afterBegin", productTemplate(this.product)
      );
    }
  }
...
function productTemplate(data) {
    ...
    <img class="divider" src="${data.Image}" alt="${data.NameWithoutBrand}"/>
    <p class="product-card__price">${data.FinalPrice}</p>
    <p class="product__color">${data.Colors[0].ColorName}</p>
    <p class="product__description"> ${data.DescriptionHtmlSimple} </p>
